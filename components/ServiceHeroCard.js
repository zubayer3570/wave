...
const ServiceHeroCard = ({ name, text, id }) => {
    return (
        <div className={`bg-gradient-to-r ${id % 2 ? "from-[#C6DBFF] to-[#E1EBFA]" : 'from-[#E1EBFA] to-[#C6DBFF]'} grid grid-cols-5 w-full rounded overflow-hidden my-8`}>
            ...
            <div className='order-1 col-span-3'>
                ...
                <Link href={`/projects/${name}`}>
                    <button className="block mt-12 bg-[#4397A4] rounded px-4 py-3 text-[white] text-[16px] font-bold shadow-2xl">See Projects on {name}</button>
                </Link>
            </div>
        </div>
    );
